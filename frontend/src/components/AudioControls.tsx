...
const AudioControls: React.FC<AudioControlsProps> = ({ className = '' }) => {
  ...
  // Sync volume and mute state with songService
  useEffect(() => {
  ...
  // Initialise with current service settings on mount and listen for changes
  useEffect(() => {
    setVolume(songService.getCurrentVolume());
    setIsMuted(songService.getCurrentMutedState());
    
    // Listen for mute state changes from the service
  ...
  const handleVolumeChange = (newVolume: number) => {
    setVolume(newVolume);
    setIsMuted(false); // Unmute when volume is changed
    songService.setVolume(newVolume);
    songService.setMuted(false);
  };

  const handleSliderClick = (e: React.MouseEvent<HTMLDivElement>) => {
    // If clicking on the slider track, unmute and set volume
    if (isMuted) {
      setIsMuted(false);
      songService.setMuted(false);
    }
